...
| QB-01 | QB | H1 | API + App | DONE | RESET-01 | Corriger la sur-generation question bank et separer session count / pool target / per-KU target. | Tests API question bank/readiness + tests App readiness si wording change. | `QB_01_QUESTION_BANK_BUDGET_REPORT.md` | Planner livre; 35/65 corriges; MODE-01 livre ensuite. |
| MODE-01 | MODE | H1 | App + API si necessaire | DONE | QB-01 | Stabiliser les cartes course-level et le wording des modes. | Tests widget course detail + tests repository si contrat change. | `MODE_01_CANONICAL_REVISION_MODES_REPORT.md` | App livree; API inchangee; exam actuel renomme `Preparation examen - QCM`. |
| RICH-01 | RICH | H1 | API + App | DONE | MODE-01 | Reexposer QCM complet depuis le cours. | Tests API rich history/start + tests App route/page/course detail. | `RICH_01_COURSE_LEVEL_QCM_COMPLET_REPORT.md` | Presets 6/10/13 livres; start course-level via session existante. |
| RICH-01B | RICH | H1 | App | DONE | RICH-01 | Transformer le QCM complet en flow sequentiel mobile-first. | Tests App rich closed exercise + activities/courses/router. | `RICH_01B_QCM_COMPLET_SEQUENTIAL_UX_REPORT.md` | Une question visible a la fois; precedent/suivant; submit final unique; result existant conserve. |
| DEEP-01A | DEEP | H2 | API + App | DONE | MODE-01 | Activer question ouverte depuis le cours. | Tests API open question ownership + tests App page/flow. | `DEEP_01A_COURSE_LEVEL_DEEP_START_REPORT.md` | Options/start/submit course-level livres; result/history deep restent pour DEEP-01B. |
| DEEP-01B | DEEP | H2 | API + App | DONE | DEEP-01A | Ajouter lifecycle, completion, result, history et reopen result deep. | Tests API session DEEP + tests App result/history. | `DEEP_01B_DEEP_RESULT_HISTORY_REPORT.md` | Result/history/reopen livres; parent DEEP peut passer DONE. |
