...
V1-022 est implémenté côté Flutter et validable: `image_choice` est ajouté aux modèles, au parser strict, au controller de réponses, au registry local allowlisté, au widget minimal, au renderer, à la correction UI, aux fixtures et tests. Aucun score n'est calculé côté Flutter. Aucune image distante, aucun `Image.network`, aucun WebView, aucun rendu JSON arbitraire, aucun champ `blob`/base64/URL/storage et aucun V1-023 n'ont été ajoutés.

Le rendu est volontairement minimal: les assets image sont résolus via un registry local fermé. Tant que de vrais fichiers image licenciés ne sont pas branchés, le widget affiche un fallback icône + libellé neutre, ce qui permet de tester le flow sans fuite ni accès réseau.
...
- Widgets rich closed existants V1-A/V1-B/V1-C.
- `lib/features/activities/application/rich_closed_exercise_flow_controller.dart`
- Tests `test/features/activities`, Today, revision sessions et router.
- `docs/v1/ROADMAP_EXECUTION_PLAN_V1.md` et rapports V1 précédents.
...
- Branche: `main`
- Status initial observé au démarrage du lot: worktree déjà non vide avec les changements V1-022 en cours dans API et app; aucun reset ni revert effectué.
- Status final avant clôture:

```text
 M docs/v1/ROADMAP_EXECUTION_PLAN_V1.md
 M lib/features/activities/domain/rich_closed_exercise.dart
 M lib/features/activities/presentation/rich_closed/rich_closed_answer_controller.dart
 M lib/features/activities/presentation/rich_closed/rich_closed_correction_presenter.dart
 M lib/features/activities/presentation/rich_closed/rich_closed_question_card.dart
 M lib/features/activities/presentation/rich_closed/rich_closed_question_renderer.dart
 M test/features/activities/fixtures/rich_closed_exercise_fixtures.dart
 M test/features/activities/http_activities_api_test.dart
 M test/features/activities/rich_closed_answer_controller_test.dart
 M test/features/activities/rich_closed_correction_presenter_test.dart
 M test/features/activities/rich_closed_exercise_flow_controller_test.dart
 M test/features/activities/rich_closed_exercise_page_test.dart
 M test/features/activities/rich_closed_exercise_test.dart
?? docs/v1/ROADMAP_EXECUTION_LOT_V1_022_IMAGE_CHOICE.md
?? lib/features/activities/presentation/rich_closed/rich_closed_image_asset_registry.dart
...
Le repo API est aussi modifié pour V1-022 et possède son propre rapport: `/Users/karim/Project/app-révision/api/docs/v1/ROADMAP_EXECUTION_LOT_V1_022_IMAGE_CHOICE.md`.

## 4. Périmètre réalisé

### Frontend app

- Ajout de `RichClosedQuestionKind.imageChoice`.
- Ajout des modèles `RichClosedImageChoiceQuestion`, `RichClosedImageChoiceOption` et `RichClosedImageChoiceAnswer`.
- Parser public strict avec rejet des corrections et des champs de rendu arbitraire.
- Controller de réponses: sélection obligatoire, remplacement de choix, ignore les IDs inconnus.
- Widget minimal `RichClosedImageChoiceWidget` avec cards sélectionnables, fallback local et sémantique accessible.
- Renderer et question card mis à jour.
- Correction UI post-submit via données backend uniquement.
- Fixtures/tests V1-D ajoutés.
- Plan V1 app mis à jour.

### Backend API

Le backend est traité dans le rapport API: contrat, validation, Genkit mockable, anti-fuite, scoring et smoke E2E.

## 5. Contrat V1-D

### image_choice

Payload public consommé par Flutter:

```json
{
  "id": "q-image-choice",
  "questionKind": "image_choice",
  "prompt": "Identifie le personnage historique associé à cet appel.",
  "instruction": "Choisis l'image correcte.",
  "difficulty": "medium",
  "cognitiveSkill": "identify",
  "sourceChunkIds": ["chunk-image-choice"],
  "choices": [
    {
      "id": "choice-image-a",
      "label": "Image A",
      "imageAssetId": "image-choice-historical-figure-001-v1",
      "altText": "Portrait historique en noir et blanc d'un homme en uniforme.",
      "caption": "Image A",
      "credit": "Placeholder interne Revision Project",
...
Réponse Flutter envoyée au backend:

```json
{
  "questionId": "q-image-choice",
  "questionKind": "image_choice",
  "choiceId": "choice-image-a"
}
```

Correction post-submit attendue:

```json
{
  "submittedChoiceId": "choice-image-b",
  "correctChoiceId": "choice-image-a",
  "explanation": "Explication backend"
}
```

Le frontend n'attend ni URL, ni base64, ni storage path, ni blob. Les IDs publics sont opaques et les libellés visibles restent neutres.

## 6. Genkit

Côté app, aucun provider IA n'est appelé. Les fixtures alignent le parser sur le contrat Genkit/API: catalogue borné, `imageAssetId` opaque, `altText` public exact, pas d'URL, pas d'image générée, pas de `blob`, pas de widget libre, pas de V1-023.

## 7. Validation/scoring

Flutter ne score pas. Il valide seulement la forme et la sélection locale avant submit:

- `choices.length` entre 2 et 6.
- IDs de choices uniques et non vides.
- `imageAssetId` présent dans le registry allowlisté.
- `altText` identique à l'alt public attendu par le registry.
- `choiceId` soumis doit appartenir à la question.
- Les champs privés ou de rendu arbitraire sont rejetés en pré-submit: `correctChoiceId`, `explanation`, `semanticLabel`, `answerHint`, `imageUrl`, `url`, `storagePath`, `base64`, `dataUri`, `blob`, `assetPath`, `renderPayload`, etc.

Le scoring exact et la tolérance d'erreur appartiennent exclusivement au backend.

## 8. Flutter

- Modèles discriminés dans `rich_closed_exercise.dart`.
- Parser strict: `image_choice` inconnu ou futur `fill_blank_dropdown` rejeté.
- Registry local: résolution allowlistée vers un éventuel `assetPath` local, actuellement `null` avec fallback déterministe.
- Widget minimal: cards, icône fallback, selected state, callback `onChanged(choiceId)`.
- Accessibilité minimale: chaque option expose un label sémantique combinant libellé neutre et alt public non révélateur.
- Correction UI: affiche le choix soumis, le choix correct et l'explication backend, sans recalcul.

Limite volontaire: pas de design final, pas de vraie galerie d'images, pas de téléchargement distant. La refonte UI pourra remplacer le fallback par des assets locaux licenciés sans changer le contrat public.

## 9. Anti-fuite

- Le parser public rejette `correctChoiceId`, `explanation`, `semanticLabel`, `answerHint` et les champs de transport image arbitraires.
- Le widget ne rend jamais JSON arbitraire et ne consomme pas d'URL.
- La correction n'est affichée qu'après submit, à partir du DTO backend.
- Les tests vérifient explicitement l'absence de correction pré-submit et l'absence de champs `blob`/URL/base64/storage.
...
- Parser/model: parse `image_choice`, rejet champs privés, rejet URL/base64/blob/storage/render, rejet asset inconnu, rejet altText incohérent, rejet type futur `fill_blank_dropdown`.
- Answer controller: état incomplet initial, sélection, remplacement, ignore choice inconnu, sérialisation sans champs interdits.
- Widget: affichage des choices, fallback, sélection, absence de correction pré-submit.
- Page/flow: renderer, submit enabled quand V1-D est répondu, correction post-submit affichée.
- HTTP API: `questionTypeMix` sérialise `image_choice`.
- Non-régression: suites activities, Today, revision sessions, router et test complet Flutter.

## 12. Validations lancées avec résultats

- `dart format <liste explicite des fichiers modifiés>`: OK.
- `flutter test test/features/activities --reporter compact`: OK, 228 tests passed.
...
- `flutter test --reporter compact`: OK, 359 tests passed.
- `git diff --check`: OK après régénération finale du rapport.

## 13. Validations non lancées avec justification

Aucune validation Flutter demandée n'a été volontairement omise. Aucun test provider IA, aucune migration, aucun `dart fix --apply`, aucun `dart format .`.

## 14. Risques restants

- Les images sont encore des placeholders logiques sans bitmap local; c'est assumé pour éviter URL/base64/storage avant cadrage légal/licence.
- L'alt public est volontairement non révélateur; l'accessibilité fine devra être reprise avec des assets réels et une stratégie pédagogique adaptée.
- Le widget est fonctionnel mais pas design final.

## 15. Recommandation prochain lot

V1-023 — Runbook demo V1 est recommandé. Aucun bis bloquant n'est nécessaire côté app pour V1-022, mais le runbook devra mentionner que les images V1-D sont encore en fallback local tant qu'aucun asset image licite n'est branché.

## 16. Passes de review

Reviews sub-agents exécutées et intégrées:

- Backend contract: a signalé le risque d'IDs/labels sémantiques publics; corrigé côté API et répercuté dans les fixtures Flutter.
- Backend Genkit/schema: a signalé le risque de fuite par label/caption; corrigé côté API, app fixtures et tests.
- Scoring/validation/public mapper: a demandé des IDs publics opaques et l'absence de champs image transport; corrigé.
- Flutter parser/model: pas de blocage; vérification du parsing strict et absence de score Flutter.
- Flutter widget: vérification du widget minimal, fallback local et absence d'URL.
- Tests/sécurité: a trouvé `blob` insuffisamment couvert; corrigé dans parser, controller, tests et docs.

Checklist finale manuelle:

- Pas de correction pré-submit.
- Pas de `correctChoiceId` pré-submit.
- Pas de `semanticLabel` pré-submit.
- Pas d'URL/base64/blob/storagePath/assetPath arbitraire.
- Pas de `Image.network`.
- Pas de widget libre.
- Pas de provider IA réel.
- Pas de V1-023 implémenté.
- Pas de score Flutter.
- V1-A/V1-B/V1-C couverts par les suites existantes.

## 17. Critique honnête du prompt initial

Le prompt est solide et nécessairement strict. Les reviews ont montré deux angles morts pratiques: le risque de fuite par IDs publics sémantiques et l'oubli du champ `blob` dans certaines listes de champs interdits. La contrainte “personnages historiques” crée aussi une tension réelle avec l'accessibilité: un alt trop précis révèle la réponse, un alt trop neutre informe moins. Le lot tranche prudemment en faveur de l'anti-fuite et documente cette limite pour la refonte UI.

## 18. Contenu complet des fichiers créés/modifiés/supprimés

Le contenu complet du présent rapport n'est pas inclus dans lui-même pour éviter une récursion infinie. Tous les autres fichiers créés ou modifiés côté app sont reproduits ci-dessous.
