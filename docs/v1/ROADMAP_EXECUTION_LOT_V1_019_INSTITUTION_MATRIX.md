...
V1-019 est implémenté côté Flutter. Le type fermé `institution_matrix` est ajouté aux modèles, au parser strict, au contrôleur de réponses, au renderer, au widget minimal et à l'UI de correction.

Aucun score n'est recalculé côté Flutter, aucun JSON arbitraire n'est rendu, aucun widget libre n'est créé et V1-020 n'a pas été traité.

## 2. Sources inspectées

- `lib/features/activities/domain/rich_closed_exercise.dart`
- Parser/API rich closed via fixtures et tests.
...
- Widgets rich closed V1-A, V1-B et V1-018.
- `rich_closed_true_false_grid_widget.dart` et `rich_closed_cause_consequence_widget.dart`.
...
- Page rich closed, tests activities, Today, revision sessions et router.
- Docs V1 app, rapports V1-017/V1-018.
...
- Derniers commits initiaux : `0fe0583 018: Intégration de la grille vrai/faux et cause/conséquence`, `347ce47 017: Intégration de la timeline et du slider de date`, `13e54e0 015-016: Intégration du type rich closed exercise`, `a3b52a9 014: Intégration des sessions de révision`, `644137b 013: Intégration des rapports Today`.
- Repo API également modifié pour ce lot ; rapport séparé créé côté `api/docs/v1`.

## 4. Périmètre réalisé

Frontend app :

- Ajout de `RichClosedQuestionKind.institutionMatrix`.
- Ajout des modèles discriminés question, cell, value, answer et correction.
- Parser public strict, avec rejet des champs privés pré-submit.
- Validation de cohérence rows/columns/cells/options, incluant les coordonnées cell dupliquées.
- Controller de réponses avec sélection nullable par cellule, complétude stricte et remplacement de valeur.
- Widget minimal groupé par ligne, dropdown fermé par cellule, sans valeur par défaut.
- Renderer et carte question mis à jour.
- Correction UI affichant valeurs soumises et valeurs correctes backend sans recalculer le score.
- Tests parser/controller/widget/page/correction renforcés.

Backend API : voir rapport API dédié.

## 5. Contrat V1-C

`institution_matrix` est une question fermée matricielle.

Payload public attendu côté Flutter : `rows`, `columns`, `cells`, chaque cell ayant des `options` fermées.

Champs privés interdits pré-submit : `correctValues`, `explanation`, `correction`, `score`, `feedback`, `modelAnswer`, tout `correct*`.

Answer submit produit :

`{ questionId, questionKind: 'institution_matrix', values: [{ cellId, optionId }] }`.

Correction post-submit parsée :

`{ correctValues: [{ cellId, optionId }] }`, affichée avec le statut et l'explication venant du backend.

## 6. Genkit

Côté Flutter, aucun provider IA n'est lancé. Le frontend ne choisit aucun widget libre et ne rend aucun JSON arbitraire. Le type `institution_matrix` arrive uniquement comme kind connu et parser strict.

## 7. Validation/scoring

Validation Flutter :

- 2 à 5 rows et columns.
- IDs rows/columns/cells uniques.
- 3 cells minimum et pas plus que `rows.length * columns.length`.
- Références rowId/columnId connues.
- Pas de coordonnée `rowId/columnId` dupliquée.
- 2 à 6 options par cell, IDs options uniques.
- Answer complète seulement si toutes les cells connues ont une option valide.
- Changement de valeur remplace la valeur précédente.

Scoring : aucun calcul côté Flutter ; le score, `isCorrect`, `partialScore` et la correction affichée viennent du backend.
...
- `RichClosedInstitutionMatrixAxisItem`
...
Widget minimal : liste groupée par ligne, cellules affichées avec le label de colonne et un `DropdownButtonFormField`. Pas de valeur par défaut ; l'utilisateur doit choisir chaque cellule. Ce choix est volontairement temporaire avant refonte UI pour éviter une table illisible sur mobile.

Correction UI : affiche les valeurs soumises et les valeurs correctes du backend, avec fallback propre si une option n'est plus trouvée.

## 9. Anti-fuite

Le parser pré-submit rejette récursivement les champs privés, y compris `correctValues` et `explanation`. Les tests vérifient l'absence de correction visible avant submit. Post-submit, la correction backend est parsée et rendue explicitement.
...
- Parser/model : parse public V1-C, rejet `correctValues`, rejet `explanation`, références row/column inconnues, coordonnées dupliquées, correction post-submit, rejet `diagram_labeling`.
- Answer controller : état incomplet initial, complétude après toutes les cells, answer `values`, ignore cell/option inconnues, remplacement de valeur.
- Widget : rows/columns/cells affichées, dropdown produit la réponse, pas de correction pré-submit.
- Page rich closed : renderer V1-C, submit complet et correction post-submit.
- Non-régression : V1-A, timeline/date_slider, true_false_grid/cause_consequence, Today, sessions, router.

## 12. Validations lancées avec résultats

- `dart format lib/features/activities/domain/rich_closed_exercise.dart test/features/activities/rich_closed_exercise_test.dart` : OK.
- `dart format <liste explicite des fichiers modifiés>` : OK lors de la passe initiale V1-C.
- `flutter test test/features/activities/rich_closed_exercise_test.dart --reporter compact` : OK, 25 tests.
- `dart analyze lib test` : OK, no issues found.
- `flutter test test/features/activities --reporter compact` : OK, 189 tests.
- `flutter test test/features/today --reporter compact` : première tentative échouée par concurrence de tooling Flutter/native assets ; relance séquentielle OK, 18 tests.
...
- `flutter test --reporter compact` : OK, 320 tests.
- `git diff --check` : OK.

## 13. Validations non lancées avec justification

Aucune validation obligatoire côté Flutter n'est restée non lancée. `dart fix --apply` et `dart format .` n'ont pas été lancés, conformément au prompt.

## 14. Risques restants

- UI volontairement minimale : dropdowns standard, pas de polish final.
- Tests widget sélectionnent via callback `DropdownButton` plutôt qu'en ouvrant systématiquement l'overlay Flutter ; le chemin d'état/réponse est couvert.
- Pas de partial score côté UI, par design ; l'affichage dépend strictement du backend.
...
- Backend contract : validé dans rapport API, remarque coordonnée dupliquée corrigée.
- Backend Genkit : validé dans rapport API.
- Backend scoring : validé dans rapport API.
- Public mapper anti-fuite : validé dans rapport API.
- Flutter parser/model : validé, strict et sans JSON arbitraire.
- Flutter widget : validé, pas de valeur par défaut, mobile-safe minimal.
- Correction UI : validée, sans recalcul de score.
- Tests : suites obligatoires relancées après correction.
- Sécurité : pas de secret, pas de provider réel, pas de widget libre, pas de V1-020.

## 17. Critique honnête du prompt initial

Le prompt était cohérent. La contrainte de contenu complet du rapport est utile pour audit, mais elle devient récursive pour le rapport lui-même ; cette section exclut donc explicitement le rapport courant et inclut tous les autres fichiers touchés côté app.

## 18. Contenu complet des fichiers créés/modifiés/supprimés

Note : le présent rapport est le seul fichier touché non recopié intégralement dans cette section, car s’inclure lui-même créerait une récursion infinie. Tous les autres fichiers créés/modifiés côté app sont inclus ci-dessous. Aucun fichier supprimé.
