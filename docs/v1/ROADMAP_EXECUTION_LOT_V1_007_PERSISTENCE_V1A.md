# LOT V1-007 — Persistance minimale V1-A

## 1. Résultat

La persistance minimale V1-A des exercices rich closed est en place côté API. Une migration Prisma additive introduit un type d’activité `RICH_CLOSED_EXERCISE`, un payload interne `RichClosedExercisePayload` et un résultat `RichClosedExerciseResult`. Le repository Prisma crée, relit et soumet ces exercices sans exposer les corrections en pré-submit.
...
- Ajout d’une persistance dédiée pour l’exercice V1-A et son résultat post-submit.
- Ajout d’une migration Prisma non destructive, non appliquée via `migrate deploy` dans ce lot.
- Extension du repository Activities pour créer, relire, soumettre et relire le résultat rich closed.
- Stockage JSON du payload interne complet et de la correction privée, avec mapping public pré-submit systématique.
- Ajout d’un scorer applicatif pur exact-match pour les six types V1-A.
- Ajout de tests repository et scorer centrés sur anti-fuite, relecture et double submit.

## 5. Architecture retenue

La persistance suit la décision ADR : une activité distincte `RICH_CLOSED_EXERCISE` reste reliée à `ActivitySession`, tandis que les structures V1-A sont stockées dans deux tables dédiées avec payload JSON typé côté application.

Le payload pré-submit n’est jamais reconstruit directement depuis le JSON interne : il passe par `toRichClosedPublicExerciseEnvelope`, qui supprime les champs de correction. Le résultat post-submit est stocké séparément afin de garder une frontière claire entre entraînement commencé et correction disponible.
...
- Tests scorer rich closed : exact-match, réponses partielles, doublons, réponses manquantes, shape invalide, champs privés interdits.
- Tests repository Prisma : création de session, relecture pré-submit sans fuite, sauvegarde/relecture résultat, double submit rejeté.
- Tests activities non-régression relancés.
...
- `npx prisma migrate deploy` : non lancé, interdit par le lot hors validation explicitement prévue ; la migration est seulement créée et validée par `prisma validate` / `prisma generate`.
- `npx prisma db push` : non lancé, interdit.
- `npx prisma migrate reset` : non lancé, interdit.
- `npm run lint` : non lancé, car ce script applique des corrections ; `npm run lint:check` a été utilisé.
...
- La migration n’a pas été appliquée sur une DB PostgreSQL jetable dans ce passage ; elle reste additive et validée statiquement.
- Le stockage JSON facilite la transition V1-A mais demandera une attention particulière si des requêtes analytiques fines sont nécessaires.
- Le scoring est volontairement exact-match ; les barèmes partiels pédagogiques pourront être enrichis plus tard.

## 11. Recommandation prochain lot

Poursuivre avec `V1-009 — Domain models Flutter V1-A` après validation de l’API publique V1-008, afin de brancher le frontend sur le contrat stable.

## 12. Passes de review

- Architecture backend : session dédiée, payload et résultat séparés.
- Prisma : migration additive, pas de suppression, pas de reset.
- Anti-fuite : mapping public pré-submit systématique et tests dédiés.
- Non-régression : QCM v3 et open question non modifiés fonctionnellement.
- Scope : aucun frontend, aucun Genkit runtime, aucun Today, aucun seed.

## 13. Critique honnête du prompt initial

Le prompt était précis sur la séparation persistance/API et sur l’anti-fuite. Le point délicat est que V1-007 et V1-008 se recouvrent naturellement dans les use cases et les tests ; le rapport indique donc explicitement les fichiers partagés. La demande de contenu complet des fichiers est utile pour review, mais rend les rapports volumineux.
